      ...
      case 'story':
        ...
        return (
          <div className="min-h-screen w-full flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-5xl mb-4 p-4 bg-black/20 backdrop-blur-sm border border-white/10 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                ...
                <div className="flex items-center gap-4 flex-wrap justify-center">
                   <button onClick={handleSave} className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 transition min-w-[120px]">
                        {showSavedMessage ? 'Saved!' : 'Save Story'}
                   </button>
                   <button onClick={handleStartNew} className="bg-red-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-600 transition">
                        Start New
                   </button>
                </div>
            </div>
            ...
            <div className="mt-4 flex gap-4">
              {currentPageIndex > 0 && <button onClick={() => setCurrentPageIndex(p => p - 1)} className="bg-white/20 backdrop-blur-sm text-white font-bold py-2 px-6 rounded-lg hover:bg-white/30 transition">Previous</button>}
              {currentPageIndex < storyPages.length - 1 && <button onClick={() => setCurrentPageIndex(p => p + 1)} className="bg-white/20 backdrop-blur-sm text-white font-bold py-2 px-6 rounded-lg hover:bg-white/30 transition">Next</button>}
            </div>
          </div>
        );
      default:
