...
const StoryViewer: React.FC<StoryViewerProps> = ({ page, onMakeChoice, onAnimateScene, onPageUpdate, isLoadingNextPage }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 p-4 md:p-8">
      <div className="relative aspect-video bg-black/30 rounded-2xl shadow-lg overflow-hidden flex items-center justify-center border border-white/10">
        {page.isVideoLoading && (
      ...
      <div className="flex flex-col justify-between text-white bg-black/20 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-white/10">
        ...
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                ...
                <button onClick={handlePlayNarration} disabled={isGeneratingAudio} className="flex items-center justify-center gap-2 bg-pink-500/80 hover:bg-pink-600 border border-pink-400 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    <PlayIcon className="w-5 h-5" />
                    {isGeneratingAudio ? 'Generating...' : 'Play Narration'}
                </button>
                 {!page.videoUrl && (
                    <div className="sm:col-span-2">
                        <button onClick={handleAnimateClick} disabled={page.isVideoLoading} className="w-full flex items-center justify-center gap-2 bg-sky-500/80 hover:bg-sky-600 border border-sky-400 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105 disabled:bg-gray-500 disabled:cursor-not-allowed">
            ...
            {isLoadingNextPage ? (
                ...
                {page.choices.map((choice, index) => (
                    ...
                    className="w-full text-left bg-purple-600/50 hover:bg-purple-700/70 border border-purple-400 text-white font-bold p-4 rounded-lg transition-transform transform hover:scale-102"
                    >
                    {choice.choiceText}
                    </button>
                ))}
