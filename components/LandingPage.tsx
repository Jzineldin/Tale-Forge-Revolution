...
const Header: React.FC = () => {
    ...
    <header className="absolute top-0 left-0 right-0 z-20 py-4 px-6 md:px-12 flex justify-between items-center">
        ...
        {/* Mobile Menu Panel */}
        <div className={`fixed top-0 right-0 h-full w-full max-w-xs bg-black/70 backdrop-blur-lg shadow-2xl z-40 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <nav className="flex flex-col items-start gap-6 p-8 pt-24 text-lg">
                ...
                <button className="bg-white/10 hover:bg-white/20 text-white py-3 px-4 rounded-lg transition-colors w-full text-left text-base">
                    Feedback
                </button>
        ...
        {isMenuOpen && <div className="fixed inset-0 bg-black/50 z-30 md:hidden" onClick={() => setIsMenuOpen(false)}></div>}
    </header>
    )
};
  ...
  return (
    <div className="min-h-screen w-full text-white">
        ...
        <section className="min-h-screen flex flex-col items-center justify-center text-center p-4 relative">
            <div className="relative z-10 flex flex-col items-center">
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
                    <button onClick={handleScrollToCreate} className="bg-yellow-400 text-black font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-yellow-300 transform hover:-translate-y-1 transition-all duration-300">
                        Create Story
                    </button>
                    <button className="bg-black/40 border border-white/20 text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-black/60 transform hover:-translate-y-1 transition-all duration-300">
                        Explore Stories
                    </button>
                     {hasSavedStory && (
                        <button onClick={onResumeStory} className="bg-purple-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:bg-purple-700 transform hover:-translate-y-1 transition-all duration-300">
        ...
        <section id="create-story" className="py-20 px-4">
             <div className="w-full max-w-lg mx-auto bg-black/20 backdrop-blur-lg rounded-2xl p-8 shadow-2xl border border-white/10">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"
                            required
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="w-full pl-4 pr-10 py-3 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 transition appearance-none"
                            >
                    ...
                    {mode === 'wizard' && (
                        <>
                           <div>
                                ...
                                <div className="relative">
                                    <select id="ageGroup" value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)} className="w-full pl-4 pr-10 py-3 rounded-lg bg-white/10 border border-white/20 text-white focus:outline-none focus:ring-2 focus:ring-yellow-400 transition appearance-none">
                             ...
                             <div>
                                ...
                                <input id="traits" type="text" value={traits} onChange={(e) => setTraits(e.target.value)} placeholder="e.g., brave, curious, loves cookies" className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"/>
                            </div>
                            <div>
                                <label htmlFor="customPrompt" className="block text-sm font-bold mb-2">Opening Scene (Optional)</label>
                                <textarea id="customPrompt" value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} rows={3} placeholder="In a cozy burrow under a great oak tree..." className="w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"></textarea>
                            </div>
                        </>
                    )}
                    ...
                    <button 
                        type="submit"
                        className="w-full flex items-center justify-center gap-2 bg-yellow-400 text-black font-bold py-4 px-6 rounded-xl shadow-lg hover:bg-yellow-300 transform hover:-translate-y-1 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <footer className="text-center p-8 text-gray-400 bg-black/20">
